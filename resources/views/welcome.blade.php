...
<body class="bg-gray-50">
    ...
    <div class="sm:fixed sm:top-0 sm:right-0 p-6 text-right z-10">
        ...
        <form action="{{ route('logout') }}" method="POST">
            @csrf
            <button type="submit" class="font-semibold text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white focus:outline focus:outline-2 focus:rounded-sm ">
                <span class="flex-1 ms-3 whitespace-nowrap">Logout</span>
            </button>
        </form>
        @else
        <a href="{{ route('posts.index') }}" class="font-semibold text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white focus:outline focus:outline-2 focus:rounded-sm ">Home</a>
        @endif
        @else
